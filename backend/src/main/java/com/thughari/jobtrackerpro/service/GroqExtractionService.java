...
@Service
@Slf4j
@ConditionalOnProperty(name = "groq.api.enabled", havingValue = "true")
public class GroqExtractionService implements AiExtractionService {
    ...
    public JobDTO extractJobFromEmail(String from, String subject, String body) {
        ...
        try {
            ...
            log.error("Groq API error: {}", e.getResponseBodyAsString());
            throw new AiQuotaExceededException("GROQ_API_ERROR", e);
        } catch (Exception e) {
            log.error("Groq AI Extraction failed or timed out", e);
            return null;
        }
    }
    ...
    public List<JobDTO> extractJobsFromBatch(List<EmailBatchItem> items) {
        ...
        try {
            ...
            log.error("Groq Bulk AI Extraction failed", e);
            return List.of();
        }
    }
    ...
    private JobDTO parseOpenAiResponse(String rawResponse) {
        try {
            ...
            if (contentText.equalsIgnoreCase("null")) {
                log.info("Groq AI determined this email is NOT a job application.");
                return null;
            }
            ...
            log.error("Failed to parse Groq AI response: {}", rawResponse);
            return null;
        }
    }
    ...
    private List<JobDTO> parseBulkOpenAiResponse(String rawResponse) {
        try {
            ...
            if (start == -1 || end == -1 || end <= start) {
                log.warn("Groq bulk response has no JSON array: {}", contentText.substring(0, Math.min(200, contentText.length())));
                return List.of();
            }
            ...
            log.error("Failed to parse Groq Bulk AI response: {}", rawResponse);
            return List.of();
        }
    }
