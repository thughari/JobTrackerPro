...
@Component({
  ...
  template: `
    <div class="flex items-center gap-2 md:gap-3 font-sans select-none group flex-shrink-0">
      ...
      @if (showText) {
        <div class="flex flex-col justify-center">
          <span class="font-bold tracking-tight text-gray-900 dark:text-white leading-none whitespace-nowrap" [class]="textClasses">
...
export class LogoComponent {
  ...
  get sizeClasses(): string {
    switch (this.size) {
      case 'sm': return 'w-7 h-7 md:w-8 md:h-8 rounded-lg';
      case 'md': return 'w-9 h-9 md:w-10 md:h-10 rounded-xl';
      case 'lg': return 'w-10 h-10 md:w-12 md:h-12 rounded-[14px]';
      case 'xl': return 'w-14 h-14 md:w-16 md:h-16 rounded-2xl';
      default: return 'w-9 h-9 md:w-10 md:h-10 rounded-xl';
    }
  }

  get iconSize(): string {
    switch (this.size) {
      case 'sm': return 'w-3.5 h-3.5 md:w-4 md:h-4';
      case 'md': return 'w-4 h-4 md:w-5 md:h-5';
      case 'lg': return 'w-5 h-5 md:w-6 md:h-6';
      case 'xl': return 'w-7 h-7 md:w-8 md:h-8';
  ...
  get textClasses(): string {
    switch (this.size) {
      ...
      case 'md': return 'text-base md:text-xl'; 
      case 'lg': return 'text-xl md:text-2xl';
      case 'xl': return 'text-2xl md:text-4xl';
      default: return 'text-base md:text-xl';
    }
  }
}